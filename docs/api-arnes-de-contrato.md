...
| Implementação | Marcação | Convite |
|---|---|---|
| `atomica` — índice único + compare-and-set | **0/5 quebras** | **0/5 quebras** |
| `ingenua` — ler-depois-gravar, sem índice | **5/5** — até **8 linhas** para o mesmo `id_cliente` | **5/5** — até **8 templates** de um convite |
| `ingenua-com-indice` — ler-depois-gravar, com índice | **5/5** — dado íntegro (1 linha), mas **500** no lugar de `duplicado` | (usa a ingênua) |
...
- Só **2 das 25 rotas**. São as duas que justificam a existência da API; as
  outras 23 seguem provadas pelos e2e contra o servidor falso — o que, para
